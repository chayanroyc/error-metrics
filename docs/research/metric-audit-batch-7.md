...
- **Dependencies and degeneracy:** constant pooled support makes the normed KSI/OVER denominator zero, but CPI avoids that denominator by requesting raw values. Identical all-zero and identical nonzero constant series both return zero because all three raw dependencies are zero. It can remain finite for unequal constant series because the raw ECDF integrals and RMSE remain defined.
...
- **Degeneracy:** constant predictions make the regression slope and `r2` `NaN`, so `NU` and `LC` become `NaN`; constant observations make `r2` and hence `LC` `NaN`. For predictions `[2,2,2]` and observations `[1,2,3]`, runtime returns finite `MSD=2/3` and `SB=0` beside `NU=NaN` and `LC=NaN`.
...
- **Zeros and signs:** negative magnitudes lose their sign. If `obs_i > 0` and the aligned normalized prediction is zero, `rel_entr` contributes `+inf`; predictions `[0,1]` against observations `[1,1]` demonstrate this. If `obs_i = 0`, that coordinate contributes zero. An all-zero vector divides by zero before `rel_entr`; the resulting `NaN` entries are then ignored by `nansum`, which can produce a misleading finite value (often zero). No smoothing is applied.
