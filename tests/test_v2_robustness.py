...
def test_linear_regression_is_undefined_for_constant_predictions():
    metrics = ErrorMetrics([2.0, 2.0, 2.0], [1.0, 2.0, 3.0])

    slope, r2 = metrics.linear_regression()

    assert np.isnan(slope)
    assert np.isnan(r2)
    assert np.isnan(metrics.lc())
...
def test_registry_rejects_different_function_for_existing_abbreviation():
    ...
    try:
        ...
        MetricRegistry._metrics.pop(abbreviation, None)
